  ...
  return (
    <section>
        { params.category === 'speakers' || params.category === 'headphones' || params.category === 'earphones' ?
            <>
            <h1 className="mt-24 h-24 flex justify-center items-center bg-audiocolor-b2 text-H4 font-semibold text-audiocolor-w3">{params.category.toUpperCase()}</h1>
            <ul className='px-6 pt-16'>
            {data.filter((cat) => cat.category == params.category).map((product)=> (
              <li key={product.name} className="flex flex-col items-center text-center justify-between gap-6 mb-24">
                <img src={`/../.${product.categoryImage.mobile}`} alt="" className="rounded-lg" />
