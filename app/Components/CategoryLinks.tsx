    ...
    <section className="w-full bg-audiocolor-w1 ">
        ...
        {organizedData().map((category) => (
            <li key={category.id} className="relative h-56 lg:h-72 flex flex-col justify-end items-center w-full">
                <img src={category.name === 'XX99 Mark I Headphones' ? MarkIPreview : category.name === 'ZX9 Speaker' ? ZX9Preview : YX1Preview} alt="" width='90' height='90' className={`${category.name === 'YX1 Wireless Earphones' && 'w-28 h-28'} absolute z-10 top-2 lg:top-8 xl:top-4 md:w-28 md:h-28 lg:h-32 lg:w-32 xl:w-40 xl:h-40`} />
                <img src={shadow} alt="" width='200' height='200' className="absolute z-10 top-16 lg:top-32"/>
